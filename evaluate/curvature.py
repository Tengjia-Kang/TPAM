...
if __name__ == '__main__':
    test_npz_clean_path = "/home/kang/SSD/Projects/Random-Walks/datasets/datasets_processed/shrec16_not_simplifed/test/471_not_changed_9216.npz"
    RW_npz_adv_path = "/home/kang/SSD/Projects/Random-Walks/attacks/shrec16_test_with_no_change/gorilla_471_before/gorilla_471_attacked_before.npz"
    Ours_npz_adv_path = "/home/kang/SSD/Projects/MyProject_attacks/test/gorilla_471_attacked.npz"
    clean_ = np.load(test_npz_clean_path,allow_pickle = True, encoding='latin1')
    adv_1 = np.load(RW_npz_adv_path,allow_pickle=True, encoding='latin1')

    clean_data = {k: v for k, v in clean_.items()}
    adv_1_data = {k: v for k, v in adv_1.items()}
    # clean_vertices = clean_['vertices']

    # visualize_and_compare(clean_data['vertices'],clean_data['faces'],adv_1_data['vertices'],adv_1_data['faces'])
    clean_pos = torch.from_numpy(clean_data['vertices'])
    adv_1_pos = torch.from_numpy(adv_1_data['vertices'])
    clean_face = torch.from_numpy(clean_data['faces'])
    adv_1_face = torch.from_numpy(adv_1_data['faces'])
    # compare(clean_pos, clean_face, adv_1_pos, adv_1_face)
    curva =  meancurvature(clean_pos, clean_face)

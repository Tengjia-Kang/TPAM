    ...
    for i in range(0, 30):
        ...
        for model_name in files_to_attack:
            ...
            if name_of_class == 'two_balls' or name_of_class == 'dino_ske':
                id = name_parts[3]
                _ = attack_single_mesh_Ours.attack_single_mesh(config=config, source_mesh=dataset_path + model_name, id=id,
...
def attack_shrec11_f4000_specify_id(config=None):
    ...
    if os.path.isfile(expected_filepath):
        _ = attack_single_mesh_Ours.attack_single_mesh(config=config, source_mesh=expected_filepath, id=id,
...
def attack_Pd_meshNet_shrec11_models(config = None):
    ...
    for name_of_class in meshCNN_and_Pd_meshNet_shrec11_labels:
        ...
        for model_name in files_to_attack:
            ...
            if name_of_class == 'two_balls' or name_of_class == 'dino_ske':
                id = name_parts[3]
                _ = Gaussian_attack_single_mesh.attack_single_mesh(config=config, source_mesh=dataset_path + model_name, id=id,
...
def attack_ExMeshCNN_shrec11_models(config = None):
    if config is None:
        return
    dataset_path = get_dataset_path(config=config)
    for name_of_class in meshCNN_and_Pd_meshNet_shrec11_labels:
